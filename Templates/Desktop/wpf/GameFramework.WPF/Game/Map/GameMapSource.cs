...
namespace GameFramework.WPF.Game.Map
{
    ...
    {
        protected GameMapSource(IServiceProvider provider, string filePath, int[,] data, ICollection<IUnit2D> units, int col, int row) : base(provider, filePath, data, units, col, row)
        {
            // NOTE: This constructor is used when creating a new map.
        }

        protected GameMapSource(IServiceProvider provider, string filePath) : base(provider, filePath)
        {
